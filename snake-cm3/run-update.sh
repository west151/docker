...
if ! [ -d $(pwd)/poky ];
 then
   ...
   if ! [ -d $(pwd)/poky/meta-snake-cm3 ];
     then
       git -C poky clone -b $YOCTO_BRANCH https://github.com/west151/meta-snake-cm3
     else
...
cp poky/meta-snake-cm3/conf/bblayers.conf.docker build/conf/bblayers.conf
cp poky/meta-snake-cm3/conf/local.conf.example build/conf/local.conf
