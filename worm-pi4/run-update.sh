...
if ! [ -d $(pwd)/poky ];
 then
   ...
   echo
   git clone -b $YOCTO_BRANCH git@github.com:west151/meta-worm-cm4.git poky/meta-worm-cm4
   echo
   ...
   echo
   git clone -b $YOCTO_BRANCH http://git.yoctoproject.org/git/meta-virtualization poky/meta-virtualization
 else
   ...
   # meta-worm-cm4
   echo
   echo -e "${CL_YELLOY}update meta-worm-cm4:${CL_NC}" $YOCTO_BRANCH
   if ! [ -d $(pwd)/poky/meta-worm-cm4 ];
     then
       git -C poky clone -b $YOCTO_BRANCH git@github.com:west151/meta-worm-cm4.git
     else
       git -C poky/meta-worm-cm4 fetch && git -C poky/meta-worm-cm4 pull origin
   fi
   # meta-openembedded
   echo
...
cp poky/meta-worm-cm4/conf/bblayers.conf.docker build/conf/bblayers.conf
cp poky/meta-worm-cm4/conf/local.conf.example build/conf/local.conf
