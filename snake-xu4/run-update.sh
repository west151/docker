...
if ! [ -d $(pwd)/poky ];
 then
   ...
   echo
   cd poky
   git clone -b $YOCTO_BRANCH https://github.com/west151/meta-snake-xu4
   echo
   git clone -b $YOCTO_BRANCH git://git.openembedded.org/meta-openembedded
   echo
   git clone -b $YOCTO_BRANCH https://github.com/akuster/meta-odroid
   echo
   ...
   #
   echo
   echo -e "${CL_YELLOY}update meta-snake-xu4:${CL_NC}" $YOCTO_BRANCH
   cd meta-snake-xu4
   git fetch && git pull origin
   #
   echo
   ...
   #
   echo
   echo -e "${CL_YELLOY}update meta-odroid:${CL_NC}" $YOCTO_BRANCH
   cd ../meta-odroid
   git fetch && git pull origin
   #
   echo
...
cp poky/meta-snake-xu4/conf/bblayers.conf.docker build/conf/bblayers.conf
cp poky/meta-snake-xu4/conf/local.conf.example build/conf/local.conf
